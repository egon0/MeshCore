...
void RadioLibWrapper::loop() {
  if (state == STATE_RX && _num_floor_samples < NUM_NOISE_FLOOR_SAMPLES) {
    ...
    // Upstream 3138ad99 (1.17.1) put this behind MESH_DEBUG_NOISE_FLOOR; adopted so debug builds
    // are not spammed. Our carried #2933 patch keeps the "(median)" tag -- it is how a flashed
    // build is told apart from mainline on the wire, so any bench run needs BOTH this define and
    // MESH_DEBUG, or it is blind on the value it is measuring.
